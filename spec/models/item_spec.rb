...
RSpec.describe Item, type: :model do
  ...
  before :each do
    ...
    @transaction7 = @invoice1.transactions.create!(credit_card_number: "7894739999", credit_card_expiration_date: '04/20', result: 0)
    @transaction8 = @invoice2.transactions.create!(credit_card_number: "7894739999", credit_card_expiration_date: '04/20', result: 0)

    @ii1 = InvoiceItem.create!(invoice_id: @invoice1.id, item_id: @item1.id, status: 1, quantity: 4, unit_price:2000)
    ...
    @ii6 = InvoiceItem.create!(invoice_id: @invoice2.id, item_id: @item1.id, status: 0, quantity: 5, unit_price: 1800)
    @ii7 = InvoiceItem.create!(invoice_id: @invoice2.id, item_id: @item1.id, status: 0, quantity: 6, unit_price: 2000)
  end
  ...
  describe 'instance methods' do
    ...
    it 'can find an items best day' do
      expect(@item1.best_day).to eq(@invoice2.created_at)
    end
  end
